...
class SupervisorAgent(BaseAgent):
    ...
    def run(self, state: ResearchState) -> ResearchState:
        ...
        if state.iteration >= self.settings.max_iterations:
            ...
            state.add_trace_event("supervisor_decision", {"route": next_route, "reason": "Max iterations reached"})
            return state
        ...
        if not state.research_notes:
            ...
            state.record_route(next_route)
            state.add_trace_event("supervisor_decision", {"route": next_route, "reason": "No research notes found"})
            return state

        if not state.analysis_notes:
            next_route = "analyst"
            state.record_route(next_route)
            state.add_trace_event("supervisor_decision", {"route": next_route, "reason": "No analysis notes found"})
            return state

        if not state.final_answer:
            next_route = "writer"
            state.record_route(next_route)
            state.add_trace_event("supervisor_decision", {"route": next_route, "reason": "No final answer found"})
            return state
        ...
        if last_agent == "writer":
            next_route = "critic"
            state.record_route(next_route)
            state.add_trace_event("supervisor_decision", {"route": next_route, "reason": "Reviewing draft with Critic"})
            return state
        ...
        if last_agent == "critic":
            ...
            if critic_results:
                ...
                try:
                    ...
                    status = "approved" if "approve" in last_critic_feedback.lower() else "needs_revision"
                ...
                if status == "approved":
                    ...
                    if "search" in feedback_lower or "missing info" in feedback_lower or "find" in feedback_lower:
                        next_route = "researcher"
                    else:
                        next_route = "writer"

                state.record_route(next_route)
                state.add_trace_event("supervisor_decision", {"route": next_route, "reason": f"Critic decision: {reason}"})
                return state

        # Default fallback
        next_route = "__end__"
        state.record_route(next_route)
        state.add_trace_event("supervisor_decision", {"route": next_route, "reason": "Default route triggered"})
        return state
